...
import { CalendarDays, UserCircle, Tag, ArrowLeft, Loader2 } from 'lucide-react';
...
export default function EducatePostPage() {
  ...
  useEffect(() => {
    if (id) {
      ...
      setPost(fetchedPost);
      if (fetchedPost) {
        document.title = `${fetchedPost.title} | MomEase Educate`;
      } else {
        document.title = "Post Not Found | MomEase Educate";
      }
    }
  }, [id]);
